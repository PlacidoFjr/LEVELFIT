      ...
      <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--text-muted)]">
            Visao para acompanhar uso, profissionais, pilotos e pendencias antes de abrir o produto para mais clientes.
          </p>
        </div>
...
function AdminMetricCard({ label, value, detail, icon: Icon, tone }: { label: string; value: string; detail: string; icon: LucideIcon; tone: keyof typeof toneClass }) {
...
function WorkspaceCard({ workspace }: { workspace: (typeof productWorkspaces)[number] }) {
  return (
    <section className="app-card premium-card p-5" data-reveal>
      ...
      <div className="mt-5 rounded-[8px] border border-[var(--border)] bg-[rgba(8,11,15,0.28)] p-3">
        <p className="text-xs font-black uppercase text-[var(--text-dim)]">Monetizacao</p>
      ...
      <RevealGroup>
        <section className="mb-5 grid gap-3 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <div className="app-card premium-card p-5" data-reveal>
            ...
            <h2 className="mt-2 text-xl font-black text-white">Comecar com 2 profissionais</h2>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {adminStats.map((stat) => <AdminMetricCard key={stat.label} {...stat} />)}
        </div>

        <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_380px]">
          <div className="grid gap-4 md:grid-cols-2">
            {productWorkspaces.map((workspace) => <WorkspaceCard key={workspace.id} workspace={workspace} />)}
          </div>
