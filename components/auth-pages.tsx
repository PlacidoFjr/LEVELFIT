...
export function LoginPage() {
  ...
  return <AuthFrame eyebrow="Bem-vinda de volta" title="Entre no seu ritmo" description="Acesse seu plano de hoje e continue de onde parou."><form onSubmit={submit} className="space-y-5"><label htmlFor="email" className="block text-sm font-bold text-[var(--text-muted)]">E-mail<input id="email" type="email" className="field mt-2" placeholder="voce@exemplo.com" required autoComplete="email" defaultValue="marina@levelfit.app" /></label><PasswordField /><div className="flex items-center justify-between gap-3"><label className="flex items-center gap-2 text-sm text-[var(--text-muted)]"><input type="checkbox" className="size-4 accent-[var(--lime)]" /> Lembrar de mim</label><button type="button" className="text-sm font-bold text-[var(--lime)]">Esqueci a senha</button></div><button type="submit" className="primary-button w-full">Entrar <ArrowRight size={18} /></button></form><p className="mt-6 text-center text-sm text-[var(--text-muted)]">Ainda nao tem conta? <Link href="/register" className="font-bold text-[var(--lime)]">Criar conta</Link></p><div className="mt-8 flex items-center justify-center gap-2 text-xs text-[var(--text-dim)]"><ShieldCheck size={15} /> Conexao segura e dados privados</div></AuthFrame>;
}

export function RegisterPage() {
  const router = useRouter();
  function submit(event: FormEvent) { event.preventDefault(); router.push("/onboarding"); }
  return <AuthFrame eyebrow="Comece com leveza" title="Crie sua conta" description="Leva menos de dois minutos. Suas preferencias podem mudar depois."><form onSubmit={submit} className="space-y-5"><label htmlFor="name" className="block text-sm font-bold text-[var(--text-muted)]">Como quer ser chamada?<input id="name" className="field mt-2" placeholder="Seu nome" required autoComplete="name" /></label><label htmlFor="register-email" className="block text-sm font-bold text-[var(--text-muted)]">E-mail<input id="register-email" type="email" className="field mt-2" placeholder="voce@exemplo.com" required autoComplete="email" /></label><PasswordField id="register-password" label="Crie uma senha" placeholder="Minimo de 10 caracteres" /><label className="flex items-start gap-3 text-xs leading-5 text-[var(--text-muted)]"><input type="checkbox" className="mt-1 size-4 shrink-0 accent-[var(--lime)]" required /> <span>Li e aceito os termos de uso e a politica de privacidade.</span></label><button type="submit" className="primary-button w-full">Continuar <ArrowRight size={18} /></button></form><p className="mt-6 text-center text-sm text-[var(--text-muted)]">Ja tem conta? <Link href="/login" className="font-bold text-[var(--lime)]">Entrar</Link></p></AuthFrame>;
}

const goals = [
  { id: "consistency", label: "Criar consistencia", detail: "Uma rotina que cabe na vida real", icon: Target, color: "var(--lime)" },
  { id: "strength", label: "Ganhar forca", detail: "Evolucao gradual e recuperacao", icon: Dumbbell, color: "var(--coral)" },
  { id: "energy", label: "Ter mais energia", detail: "Movimento, sono e hidratacao", icon: HeartPulse, color: "var(--cyan)" },
  { id: "nutrition", label: "Comer com mais equilibrio", detail: "Variedade sem dieta extrema", icon: Salad, color: "var(--green)" },
];
...
export function OnboardingPage() {
  ...
  return <main className="min-h-screen px-4 py-6 sm:px-8"><div className="mx-auto max-w-3xl"><div className="flex items-center justify-between"><AuthBrand /><span className="text-sm font-bold text-[var(--text-muted)]">Etapa {step} de 3</span></div><div className="mt-6 grid grid-cols-3 gap-2" aria-label={`Etapa ${step} de 3`}>{[1, 2, 3].map((item) => <span key={item} className={`h-1.5 rounded-[3px] ${item <= step ? "bg-[var(--lime)]" : "bg-[var(--surface-soft)]"}`} />)}</div>
    <section className="mt-10"><p className="eyebrow text-[var(--lime)]">{step === 1 ? "Seu foco" : step === 2 ? "Seu momento" : "Sua rotina"}</p><h1 className="mt-3 text-3xl font-black text-white">{step === 1 ? "O que voce quer construir?" : step === 2 ? "Como esta sua atividade hoje?" : "Quando prefere receber um lembrete?"}</h1><p className="mt-3 text-sm leading-6 text-[var(--text-muted)]">{step === 1 ? "Escolha o objetivo que mais importa agora. Ele pode mudar depois." : step === 2 ? "Isso ajuda a sugerir um primeiro plano confortavel." : "Lembretes sao opcionais e respeitam seu horario silencioso."}</p></section>
